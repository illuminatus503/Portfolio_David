...
function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate form properly - recalculate errors
    const currentErrors = window.validateForm ? window.validateForm(formData) : {};
    ...
    // Check required fields manually as fallback
    ...
    try {
      ...
      const submitData = {
        ...
        subject: formData.subject.trim() || 'Mensaje desde portfolio',
      ...
      const apiEndpoint = `${API_BASE}/contact`;
      ...
      try {
        ...
        console.log('💾 Stored in localStorage:', submissions);
        
        setFormStatus({ 
          type: 'success', 
          message: '✅ Mensaje enviado correctamente (modo desarrollo). El mensaje se ha guardado localmente.' 
        });
        resetForm();
      }
    } catch (error) {
      ...
      let errorMessage = translations.error || 'Error al enviar el mensaje';
      
      if (error.name === 'NetworkError' || !navigator.onLine) {
        errorMessage = translations.networkError || 'Error de conexión';
      } else if (error.message.includes('429')) {
        errorMessage = translations.rateLimitError || 'Demasiadas solicitudes';
      } else if (error.message.includes('5')) {
        errorMessage = translations.serverError || 'Error del servidor';
      }
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <a href={`mailto:${window.environment?.personal?.email || 'david.fdezcuenca@gmail.com'}`} className="text-accent hover:underline">
                    {window.environment?.personal?.email || 'david.fdezcuenca@gmail.com'}
                  </a>
                </div>
              </div>
            ...
            <div className="pt-6">
              ...
              <div className="flex space-x-4">
                <a 
                  ...
                  className="w-12 h-12 bg-secondary rounded-full flex items-center justify-center hover:bg-accent/20 transition group"
                  aria-label="LinkedIn"
                  title="LinkedIn"
                >
